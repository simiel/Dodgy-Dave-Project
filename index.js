  ...
  try {
    const stockData = await Promise.all(
      tickersArr.map(async (ticker) => {
        const url = `https://test-polygon-api-worker.simieldev.workers.dev//?ticker=${ticker}&startDate=${dates.startDate}&endDate=${dates.endDate}`;
...
async function fetchReport(data) {
  ...
  try {
    const url = "https://openai-api-worker.simieldev.workers.dev/";
